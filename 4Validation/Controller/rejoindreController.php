...
require_once __DIR__ . '/../Model/rejoindreModel.php';
...
class RejoindreController {
    ...
    public function getParticipation(int $id): ?array {
        try {
            ...
            error_log("Erreur lors de la récupération de la participation : " . $e->getMessage());
            return null;
        }
    }
    ...
    public function updateStatus(int $id, string $status): bool {
        try {
            ...
            $stmt = $this->db->prepare("UPDATE rejoindre SET statut_participation = ? WHERE id_participation = ?");
    ...
    public function sendConfirmationEmailForConfirmedParticipation($id_participation) {
        ...
        if ($data) {
            ...
            try {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = 'ghribimedaziz007@gmail.com';
                $mail->Password = 'tpwxzuxnmmbmssqi'; // ⚠️ Sécurisez ce mot de passe
    ...
    public function deleteParticipation(int $id): bool {
        try {
            $stmt = $this->db->prepare("DELETE FROM rejoindre WHERE id_participation = ?");
            return $stmt->execute([$id]);
        } catch (PDOException $e) {
            error_log("Erreur lors de la suppression de la participation : " . $e->getMessage());
            return false;
        }
    }
    ...
    public function getEventParticipations(int $eventId): array {
        try {
            ...
            error_log("Erreur lors de la récupération des participations à l'événement : " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getUserParticipations(int $userId): array {
        try {
            ...
            error_log("Erreur lors de la récupération des participations de l'utilisateur : " . $e->getMessage());
            return [];
        }
    }
}
